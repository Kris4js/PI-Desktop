...
| D128 amendment (2026-09-11) | Workspace edits preserve work-panel focus | **Supersedes D128's automatic Review activation clause: successful workspace Write/Edit results retain their D180 message-owned review evidence but never create a panel tab, reveal the panel, or change its active tab in any session. Review remains available through the user-operated launcher and retained tabs. Explicit file, plan, and BrowserPreview navigation is unchanged.** | Agent edits must not interrupt a terminal or another selected plugin view, nor reveal a collapsed panel. See `04-ux/01-ui-ia.md`, `04-ux/08-component-spec.md`, and E2E-057. |
